...
public class Hw1TalkActivity extends AppCompatActivity implements Hw1Contract.View, TextView.OnEditorActionListener, OnRecallMessageRequested {
    ...
    private boolean markdownImageFormat(String str) {
        String pattern = "!\\[.*?\\]\\(.*?\\)";
    ...
    public void showMessageList(List<ClientMessage> messages) {
        runOnUiThread(() -> {
                    ...
                    for (ClientMessage message : messages) {
                        ...
                        // 判斷是Markdown的圖片
                        if (markdownImageFormat(text.trim())) {
                            String imageURL = text.substring(text.indexOf('(') + 1, text.length() - 1);
//                            System.out.println(imageURL);
                            if (message.getSenderUsername().equals(this.presenter.getUsername())) {
                                content.addView(new ItemImageSend(this, imageURL, message.getMessageId(), this));
                            } else {
                                content.addView(new ItemImageReceive(this, imageURL, message.getMessageId()));
                            }
                        } else {
                            if (message.getSenderUsername().equals(this.presenter.getUsername())) {
